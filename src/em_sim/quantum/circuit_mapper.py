...
from qiskit.opflow import PauliOp, Z, X, Y, I
...
class MaxwellCircuitMapper:
    ...
    def _hardware_optimize(self, circ: QuantumCircuit, hw_config) -> QuantumCircuit:
        ...
        if hw_config.architecture == "superconducting":
            ...
            return transpile(circ, 
                          coupling_map=None,
                          basis_gates=['rxx', 'rz', 'ry'])
        return circ

    def map_cavity_modes(self, dimensions: List[float], 
                       boundary_conditions: Dict[str, str]) -> QuantumCircuit:
        ...
        self._apply_boundary_conditions(qc, boundary_conditions.get('type', 'PEC'))
        
        # Generate TE/TM Hamiltonians
        for qubit in range(self.config.qubit_count - 2):
            qc.append(
                PauliEvolutionGate(self.cavity_hamiltonian['TE'], time=dimensions[0]/2),
    ...
    def map_waveguide_modes(self, cross_section: List[float],
                          material_properties: Dict[str, float]) -> QuantumCircuit:
        ...
        waveguide_op = FermiOp(
            ...
            [(f'Y_{i}', (5 * (1/z0))/2) for i in range(self.config.qubit_count)]
        )
        
        qc.append(
            PauliEvolutionGate(waveguide_op.to_pauli_op(), time=cross_section[0]),
            range(self.config.qubit_count)
        )
        
        return qc

    def map_topological_boundary(self, surface_geometry: np.ndarray,
                               correlation_map: np.ndarray) -> QuantumCircuit:
        ...
        for i in range(surface_geometry.shape[0]):
            qc.rzx(surface_geometry[i,0], i%self.config.qubit_count, 
