...
class QuantumClassicalBridge:
    ...
    def __init__(self, hilbert_dim: int = 64, device: str = "cuda"):
    ...
    def classical_to_quantum(
        self, classical_field: Dict[str, torch.Tensor]
    ) -> torch.Tensor:
        ...
        quantum_state = (
            torch.sqrt(alpha) * self.basis_states[0]
    ...
    def quantum_to_classical(
        self, quantum_state: torch.Tensor, grid_size: Tuple[int, int, int]
        ...
        shape = (*grid_size, 3)
        E = torch.randn(shape, device=self.device) * torch.sqrt(proj_0)
        B = torch.randn(shape, device=self.device) * torch.sqrt(proj_1)
